import React from "react";

const SvgComponent = (props) => (
  <svg width="1em" height="1em" {...props}>
    <path d="M17.5 0C7.835 0 0 7.835 0 17.5S7.835 35 17.5 35 35 27.165 35 17.5 27.165 0 17.5 0zm0 2C26.06 2 33 8.94 33 17.5 33 26.06 26.06 33 17.5 33 8.94 33 2 26.06 2 17.5 2 8.94 8.94 2 17.5 2z" />
    <path d="M12.906 2.688c-.653.203-1.293.465-1.906.75C12.288 5.553 14.702 7 17.5 7s5.212-1.446 6.5-3.563a15.554 15.554 0 00-1.906-.75C21.115 4.066 19.437 5 17.5 5c-1.937 0-3.615-.935-4.594-2.313z" />
  </svg>
);

export default SvgComponent;
